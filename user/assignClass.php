	...
	?>
		...
		<div class="container-fluid">
			...
			<div id="feedback" class="col-lg-10 jumbotron col-md-6 col-sm-12 graybox">
				<h3 class="text-info">Assign subject</h3>
				<?php
					include '../connect.inc.php';
					if(isset($_POST['semester'])&&isset($_POST['department'])&&isset($_POST['subject_code']))
					{
						$semester = $_POST['semester'];
						$department = $_POST['department'];
						$subject_code = $_POST['subject_code'];
						if(!empty($semester)&&!empty($department)&&!empty($subject_code))
						{
							$session_year =  date('Y');
							if(date('m')<6)
								$session_sem = 'MAY';
								...
								$session_sem = 'DEC';
							$query = "insert into student_class (id, subject_code, department, semester, session_year, session_sem)
							values ( null, '$subject_code', '$department', '$semester', '$session_year', '$session_sem')";
							$run_query = mysql_query($query);
							if($run_query)
							{
								echo '<h3 class="text-success">Subject Assigned to the Class.</h3>';
							}
							else
							{
				...
				<div class="registration in-form">
					<form action="assignClass.php" method="POST" enctype="multipart/form-data">
